...
const ProjectModal: React.FC<ProjectModalProps> = ({ project, isOpen, onClose }) => {
    ...
    return (
        ...
        >
            ...
            <div
                className={`relative w-full max-w-5xl transition-all duration-700 transform ${isOpen ? 'translate-y-0 scale-100' : 'translate-y-12 scale-95'
                    }`}
            >
                {/* Close Button - Positioned inside the modal for better alignment */}
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 z-[110] p-2 bg-black/40 hover:bg-black/60 text-white rounded-lg transition-colors backdrop-blur-md border border-white/10"
                >
                    <X size={20} className="md:w-6 md:h-6" />
                </button>

                {/* Scrollable Content Card */}
                <div className="bg-[#262626] border border-white/10 rounded-2xl overflow-hidden shadow-2xl max-h-[85vh] md:max-h-[none] overflow-y-auto md:overflow-visible">
                    <div className="grid md:grid-cols-2">
                        {/* Image Section */}
                        <div className="relative h-[220px] sm:h-[280px] md:h-auto overflow-hidden group bg-black/20 flex items-center justify-center">
                            <img
                                src={project.img}
                                alt={project.title}
                        ...
                        {/* Details Section */}
                        <div className="p-6 md:p-12 flex flex-col justify-center bg-[#262626]">
                            ...
                            <p className="text-white/85 font-light leading-relaxed mb-8 md:mb-10 text-base md:text-lg">
